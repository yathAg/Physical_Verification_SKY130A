...
if __name__ == '__main__':
    ...
    if len(arguments) > 1:
        print('Usage:  count_lvs.py [filename]')
        sys.exit(1)
    elif len(arguments) == 1:
        compfile = arguments[0]
        if os.path.splitext(compfile)[1] != '.json':
            compfile = compfile + '.json'
    else:
        if os.path.isfile('comp.json'):
            compfile = 'comp.json'
        else:
            jlist = glob.glob('*.json')
            if len(jlist) == 1:
                compfile = jlist[0]
            else:
                print('No JSON file given and no JSON file found.')
                print('Usage:  count_lvs.py [filename]')
                sys.exit(1)

    if not os.path.exists(compfile):
        print('JSON file ' + compfile + ' not found.')
        print('Usage:  count_lvs.py [filename]')
        sys.exit(1)
    ...
    if total > 0:
        failed = True
        print('LVS file ' + compfile + ' reports:')
